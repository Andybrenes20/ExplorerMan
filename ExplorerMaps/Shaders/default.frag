...
void main()
{
	...
	float sunKey = pow(lambert, 0.66f) * diffuseIntensity * (1.16f + dayFactor * 0.46f) * mix(1.0f, 0.46f, rain);
	float ambientStrength = mix(0.68f + hemi * 0.34f, 0.26f + hemi * 0.26f, dayFactor);
	vec3 ambientTerm = ambientColor * ambientStrength;
	vec3 directTerm = lightColor.rgb * (sunKey + hemi * 0.035f * dayFactor);
	...
	float shadowSide = (1.0f - lambert) * (1.0f - hemi * 0.22f) * dayFactor * (1.0f + rain * 0.42f);
	float contactShadow = (facadeMask * (1.0f - hemi) * 0.34f + brightGroundMask * detailEdges * 0.70f + detailEdges * facadeMask * 0.45f) * dayDepth;
	litColor *= 1.0f - shadowSide * 0.32f - contactShadow * 0.34f;
	litColor += vec3(0.022f, 0.030f, 0.045f) * shadowSide;
	...
	litColor += skyBounceColor * fresnel * glassMask * (0.30f + 0.58f * dayFactor);
	...
	litColor = clamp((litColor - 0.5f) * 1.16f + 0.5f, 0.0f, 1.0f);
